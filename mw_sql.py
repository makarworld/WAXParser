...
def ensure_connection(func):
    ...
    def inner(*args, **kwargs):
        ...
        with sqlite3.connect(file) as connection:
            res = func(*args, conn=connection, **kwargs)
            connection.commit()
        return res
    return inner
...
class baseUniversal():
    @ensure_connection
    def __init__(self, filename, logname="database", conn=None):
    ...
    @ensure_connection
    def create(self, table: str, columns: list, types: str="TEXT", conn=None):
        cursor = conn.cursor()
    ...
    @ensure_connection
    def get_table(self, table, conn=None):
        cursor = conn.cursor()
    ...
    @ensure_connection
    def edit_by(self, table: str, edit_by: list, conn=None, **kwargs): # base.edit_by(table, ename=values, **kwargs)
        cursor = conn.cursor()
    ...
    @ensure_connection
    def get_by(self, table: str, get_by: list, args: list, order_by: str=None, conn=None):
        cursor = conn.cursor()
        if len(get_by[0]) == 0:
            return {table: []} 
    ...
    @ensure_connection
    def add(self, table: str, conn=None, **kwargs):
        cursor = conn.cursor()
    ...
    @ensure_connection
    def execute(self, command, args=None, conn=None):
        cursor = conn.cursor()
